...
- **Two-level structure: product family → variants.**
  ...
  - Since stage 10 a supplier can also enter families and variants itself, in the same shape (§9, D59).
...
4. **Comparison (hub, on the requirement):**
   ...
   - **Statuses.** `MATCH`, `ACCEPTABLE_DEVIATION`, `MISMATCH` and `UNKNOWN` are the four the judge may also return. Three more come only from the comparators:
     ...
     - `NEEDS_JUDGE` — deliberately left undecided: `semantic` rules, any pair whose value types don't line up (an enum against free text), and **free text worded differently** under the `exact` rule, so a spelling variant can never become a false critical mismatch.
   - **Free text is compared by meaning (D57).** The core first compares a canonical form: casefolded, punctuation and spacing collapsed, decimal commas as points, and every whole-answer "nothing" (`keine`, `nein`, `ohne`, `none`, `n/a`, `–` …) as one token, so the hospital's "nein" matches the supplier's "keine" with no model at all. Text that still differs is left `NEEDS_JUDGE` ("worded differently"); before the judge, **one `compare_text` call per round** (Haiku 4.5, no thinking) reads all such pairs and returns MATCH or MISMATCH with a rationale (`decided_by = LLM`). What it cannot tell, or a failed call, stays for the Opus judge.
...
- **Suppliers maintain their own catalog (stage 8).** In the supplier app a supplier sets a value for a whole family or overrides it for one variant, or marks it "not available". Each edit is a supplier fact like an answer (`SUPPLIER_ANSWER` or `UNAVAILABLE`, no `answer_id`, `created_by` = the user), so it outranks catalog data, and the family's projection is rebuilt at once. A supplier may **withdraw** its own facts (`withdrawn_at`, never deleted); the catalog or family value then shows again. Catalog and extraction facts cannot be withdrawn. Open assessments see an edit in their next round; nothing re-runs by itself.
- **Suppliers maintain their families and variants (stage 10, D59).** Catalog data no longer comes only from printed catalogs:
  - **New family:** name, manufacturer, brand, product type, description, properties text and a **category the supplier chooses** (`category_source = SUPPLIER`, which a reading never overrides). It is stored in the shape of a printed entry (`raw`, `created_by` = the user): the parsers read the headline at once, and a `NORMALIZE_ITEM` job (dedupe key `normalize:{family}:{content_hash}`, the one assessment creation uses) reads the description in the background. The page shows "being read" until `normalized_hash = content_hash`.
  - **New variant:** entered as its size-table row — article no. (unique per supplier; a retired number is reactivated, not reused), label, order unit and pack sizes, a size text such as "20 ml", optional GTIN (check digit enforced) and PZN. The row becomes a `source_row` (`Art.-Nr.`, `Größe`, `GTIN`, `PZN`) and is read by `variant_facts` exactly like a printed row, so these are catalog facts; everything else is set as the supplier's own values (above). The form can start as a copy of a sibling, without its article no., GTIN and PZN.
  - **Text edit:** a changed text gets a new `content_hash`; the family-scope readings of the old text (`CATALOG` and `EXTRACTION` facts) are withdrawn, the headline is parsed again and the description queued for reading, so old and new text never mix. The supplier's own values stay.
  - **Category change:** the family is re-projected under the new template; open assessments keep judging with their own `template_code`, so nothing breaks mid-loop.
  - **Retire / reactivate a variant** (`is_active`): a retired article leaves search and new assessments (404), open assessments keep it. Nothing is ever deleted.
- **Sources are visible everywhere:** every value returned by either service carries its fact ID, source, quote, author and time. `GET /supplier/catalog/families/{id}` (hub) returns a family's own values and each variant's effective value with its source and scope; `GET /articles/{id}` (node) the article's.
...
| `compare_text` | hub | **claude-haiku-4-5** | no thinking | ours | the round's free-text pairs worded differently (attribute label, hospital value, supplier value) → MATCH / MISMATCH / UNKNOWN with a rationale, one call per round, before the judge (D57) |
...
- **Supplier:**
  ...
  - `GET /supplier/catalog`, `GET /supplier/catalog/families/{id}` (template attributes, the family's values, each variant's effective values with source and scope, the supplier's own facts)
  - `PUT /supplier/catalog/facts {family_id | variant_id, attribute_key, value | unavailable}` (a value for the family, or an override for one variant; validated against the template), `DELETE /supplier/catalog/facts/{fact_id}` (withdraw an own fact)
  - `GET /supplier/catalog` answers with each family as its **size table**: the attributes whose value differs between its variants (the shared ones belong to the family page), and per article its pack and how many critical or major attributes are still unanswered
  - stage 10: `POST /supplier/catalog/families {name, manufacturer, brand_name?, product_type?, description?, properties_text?, category_code}`, `PATCH /supplier/catalog/families/{id} {text?, category_code?}`, `POST /supplier/catalog/families/{id}/variants {article_no, label, size_text?, order_unit?, units_per_order_unit?, order_units_per_shipping_unit?, gtin?, pzn?}`, `POST /supplier/catalog/variants/{id}/retire|reactivate`; each answers with the family detail (which now carries the family's text, `reading`, and per variant `is_active` and its row); another supplier's family or variant is a 404
...
- **Supplier app (served by the hub):** inbox (hospital alias only); request form with a typed input per question from `expected_answer`, comment, "applies to the whole family", "cannot provide", drafts and submit (the hub's 422 lists unanswered questions); in development, **Let the simulator answer** for the supplier's own request; catalog: the list shows each family as its size table (only what tells its articles apart, the pack, and what is still missing); each family's page shows the family's values and one variant's effective values with source and scope; the supplier edits a family value, overrides it for one variant, marks it not available, or withdraws its own value; since stage 10 the supplier also creates a family (**New family**, category chosen from the templates), edits its text and category (**Edit family**, with a note that the text is read again and a **being read** badge while it is), adds a variant (**Add variant**, optionally copied from a sibling) and retires or reactivates one (retired variants stay listed, marked).
...
| 10 | Suppliers maintain families and variants (D59): add a family and variants, edit family text and category, retire and reactivate variants | 1 | 9 | BD adds a family and a 20 ml variant that is searchable at once with its volume parsed; the description is read in the background; an edited text replaces the old readings; a retired variant leaves search; `make ui-e2e` green |
...
| D59 | **Suppliers enter families and variants in the shape of printed catalog data** (user decision, stage 10): stored as a catalog entry with `created_by`, read by the same parsers and `normalize_item`; a text edit withdraws the old readings before the new text is read; variants are retired, never deleted | A separate "supplier-entered" source ranked apart from catalog data; entering every attribute as a supplier answer; hard deletes | One path for printed and entered data keeps parsers, precedence and projection unchanged, and the supplier's own value overrides (stage 8) stay above both. Withdrawing the old readings is what lets `normalize_item` (which only fills missing keys) take a new text into account. Retiring keeps open assessments and history intact. | Suppliers need bulk import (CSV/BMEcat) → the same `store_family`/`add_variant` path behind an upload (~1 day) |
