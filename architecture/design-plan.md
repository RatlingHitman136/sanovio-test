> **Design-phase plan (archived).** This is the planning document the design was worked out in: client questions, review rounds, decisions, implementation chunks and the pre-implementation review. The current reference is [ARCHITECTURE.md](ARCHITECTURE.md); this file is kept for history and rationale.

# Article Equivalence Loop: Prototype Plan (v3, hospital node + central supplier hub)

## Context

The client wants a two-sided web app:
- **Hospital purchasers** compare their current articles with items in a supplier's catalog.
- **An LLM** decides whether the two are equivalent and lists the data it is missing.
- **Questions** go out, and the answers **enrich the records**.
- **The loop repeats** until the assessment is resolved.

`/home/ratlinghitman/code_stuff/sanovio` has no application code yet (only `charts/`). The client provided example data in `data_examples/`:
- a hospital article list (CSV)
- two supplier product catalogs (PDF)

**No code is prototyped here.** This document covers: client questions, what the example data tells us, stack, architecture, demo data, reasoning behind every choice the brief didn't fix, the implementation plan, and verification.

**Fixed by the user:**
- FastAPI (Python) backend now; a React/TypeScript frontend in a later phase.
- **uv manages Python and all Python libraries.**
- **Tiered LLM models** to save cost.
- **Backend only for now.** Everything is done with REST requests (Swagger UI, curl, a scripted demo client). The React frontend (§3.2, §4.11) is a later phase.
- **Data ingestion is out of scope.** The example files show *what data looks like*, not how it will arrive. Seeds are hand-written JSON in the canonical format.
- **The starting point is an existing hospital article.** Free-text requirement entry is not planned for now.
- **Split architecture (review round 3):**
  - **Hospital node:** one per hospital, holding that hospital's articles. The hospital may host it on its own server behind a firewall that blocks access from outside.
  - **Supplier hub:** hosted centrally by us for all hospitals and suppliers.
  - **Minimal data exchange, and only through the client.** Example: the purchaser selects an article, the client gets the important fields from the node and sends a search request for it to the hub (§4.13).
  - **Access keys stored deliberately** on each side (§4.15).
- The machine has Python 3.14 + uv, Java 25 (charts), but **no Node, nvm or Docker**.

**Revision notes (review round 3: split architecture):**
- The monolith becomes **two FastAPI services plus a shared core library** in one uv workspace (§4.1, §4.2):
  - `hospital-node`: articles, identifiers, article facts, normalization, egress log. (The node's copy of assessments was removed in round 6, D49.)
  - `supplier-hub`: catalogs, supplier facts, candidate search, assessments, questions, answers, judging, the Anthropic key.
  - `equivalence-core`: templates, parsers, comparators, fact precedence, record hash, envelope and token formats.
- **The node and the hub never connect to each other.** The client (demo CLI now, SPA later) is the only bridge (§4.14, §4.16).
- **Hospital data leaves the node only as a signed requirement envelope:** template attribute values plus an opaque `article_ref`, built by an allowlist at the node and logged there. It carries no name, brand, internal ID, GTIN/EAN, price or quantities (§4.14).
- **Purchasers reach the hub through token exchange:** the node signs a 5-minute assertion with the hospital's private Ed25519 key, and the hub checks it with the registered public key and issues a short hub token. The hub stores no hospital passwords and no shared secrets (§4.15).
- **The hub signs what the node stores** (reference product snapshots, final verdicts). The node checks them with the hub's pinned public key.
- **Judging stays central,** on the envelope. The tradeoff is stated in D35.
- **Purchaser-addressed questions** are answered at the node and reach the hub as a new envelope (§4.7, §4.12).
- **Suppliers see a pseudonymous hospital alias** by default.
- **Normalization of hospital articles runs at the node:** rule-based by default (no key); an LLM is optional with the hospital's own key.
- Appendix A is split into node tables (N.*) and hub tables (H.*). Phases grew to ~6½ days *at the time* (superseded; see §8). New client questions in Part C. New decisions D32–D42.
- **The existing charts show the monolith.** They are regenerated as the first implementation step (Appendix B).

**Revision notes (review round 4: attribute registry):**
- **The hub owns the attribute registry and the category templates** (§4.3.1). The YAML files in the core become only the seed for version 1.0.
- **Attributes can be added at runtime.** A question that no attribute covers gets an LLM proposal. The attribute is **provisional** once sent, and the supplier's answer is **visible to every hospital immediately** as additional information, with no effect on verdicts. An operator sets criticality and publishes a new template version.
- **Nodes receive hub-signed template bundles through the client.** Assessments are pinned to their template version; the hub accepts the current and the previous version.
- New tables: N.11 and H.20–H.22. New decisions D43–D45. Client questions Q20–Q21. Phases grow to ~7½ days. Scenario 7.

**Revision notes (review round 5: simplification after security research):**
- **One candidate search.** The separate reference-link lookup is gone. Every search result has two actions:
  - **"this is our current product":** preview with conflicts, values stored at the node, then an automatic **second search on the extended parameter set** (the enriched requirement)

  (§4.4, §4.13, D46)
- **Plain requirement instead of a signed envelope.** The node builds a JSON requirement through the allowlist; the hub validates it strictly against the template and takes the hospital from the token. No envelope signature and no `attributes_hash` claim (§4.14, D34).
- **No hub-signed snapshots or verdicts.** Current-product values and final verdicts are stored at the node as reported by the client (D37).
- **Remaining signatures:** hub assertions (node key) and template bundles (hub key), with RFC 8725 checks (§4.15).
- **Product hints** (brand, manufacturer article no., valid GTIN) are a per-hospital setting, off by default. Prices, quantities, internal IDs and names never leave (D47, client question 15).
- **Per-user rate limits and alerts** on requirement and assertion issuance at the node (D48).
- **Renames:**
  - node table `outbound_envelopes` → `egress_log`
  - hub table `requirement_envelopes` → `requirements`
  - template flag `envelope` → `shareable`
  - node setting `ENVELOPE_DENY_ATTRIBUTES` → `EGRESS_DENY_ATTRIBUTES`
- Estimate ≈7 days at the time (superseded; see §8).
- Round 3 notes above that mention signed envelopes, snapshots or attestations are superseded by this round.

**Revision notes (review round 6: no assessment copy at the node):**
- **Node table `assessment_links` is removed** (N.8). The hub already stores every assessment under the hospital tenant, so all purchasers of that hospital see the same list there.
- **Assignee moves to the hub** as a pseudonymous principal (`assessments.assigned_to_principal_id`, set by `subject_id`). The hub keeps pseudonyms only.
- **The client joins names locally:**
  - article names via node `GET /articles?article_ref=…`
  - user names via node `GET /users` (display name + `hub_subject_id`)
- **Verdicts are no longer reported back to the node.** A hospital-side decision archive is a later export (D49).
- Estimate ≈6¾ days at the time (superseded; see §8).

**Revision notes (review round 7: identifiers merged into facts, user decision):**
- **`article_identifiers` (N.4) and `item_identifiers` (H.9) are removed.** An identifier is a fact with a new typed value shape `{"type":"identifier","scheme":"GTIN","value":"04040456781234","checksum_valid":true}`, so it inherits the fact table's provenance: source, author, `answer_id` / `hub_question_id`, and supersedence instead of overwrites.
- **Projections keep identifiers in their own `identifiers` section** — a list per scheme, not one winner per key — so the "one effective value per attribute" invariant of `attributes` is untouched (an item can hold a base-unit and a carton GTIN).
- **Identifier definitions live in the registry with `kind = IDENTIFIER`** and are never listed in a category template, so they can never be given a criticality or a comparison rule.
- **New hub stage `identifier_evidence`, before the comparators (D51).** A check-digit-valid GTIN match — or manufacturer article no. + manufacturer — on both sides yields `SAME_TRADE_ITEM` and short-circuits the round to EQUIVALENT with no judge call. Anything else yields `NO_INFORMATION`: identifier facts never produce a MISMATCH, never become unknowns, never become questions and never enter the judge prompt. This is the asymmetry that makes identifiers useful: a match is near-proof, a difference is silence.
- **Egress is unchanged in effect, enforced differently:** `RequirementPayload` has no identifiers field and the builder maps only the projection's `attributes`; product hints remain a separate opt-in function (D47). A test asserts that no identifier value appears in any requirement while hints are off.
- **Questions need no special column:** an identifier question is a normal question whose attribute is an identifier definition and whose `expected_answer` is `{"type":"identifier","scheme":"GTIN"}`.

**Status:** rounds 5–10 are applied and verified. §10 records the pre-implementation review (D52–D55); D42 was then revised and D56 added so the node normalizes articles with the hospital's own LLM key **once, at ingestion**, with no job queue. `architecture/data-model.md` and all 21 diagrams match the plan (render clean, stale-term sweep clean). Next step: chunk 0 of §8. No backend code yet.

**Approach to open questions:** every open question has a stated default (§7). The decisions table (§6) says what changes if the client answers differently.

---

## 1. Client message (ready to send)

> If Parts A and B were already sent, send only **Part C**.

> **Subject: Article equivalence challenge: clarifying questions**
>
> Hello,
>
> Thank you for the challenge and the example data. For each question we've noted our working assumption. If it fits, a simple "OK" is enough.
>
...
## 2. What the example data tells us (structure only)

### 2.1 Hospital article list: `sample-challenge-v01(Tabelle3).csv`
...
### 2.2 Supplier catalogs: `product_catalog_01.pdf` (B. Braun, 43 pages), `product_catalog_02.pdf` (BD, 22 pages)
...
### 2.3 Where the two sides overlap
Only **syringes** (CSV row 3) and **hypodermic needles** (CSV row 6) appear in both. The B. Braun catalog holds the hospital's *current* products; BD offers the *competitor alternatives*. Demo scenarios (§5) are built on these pairs.

---

## 3. Tech stack

### 3.1 Backend: one uv workspace, two services, one core library
| Concern | Choice |
|---|---|
...
| Category templates | **Hub registry** (database tables; §4.3.1). Seeded from YAML (`pyyaml`) in the core. The core holds the Pydantic `TemplateDefinition` model and loader. The client syncs the current definition per category to each node as plain JSON (D52). |
...
| Auth | **Node:** local accounts (`pwdlib[argon2]`), opaque bearer tokens hashed in the node DB. **Hub:** supplier and operator accounts the same way; purchasers only via **token exchange** (§4.15). `HTTPBearer` dependencies check role and tenant. |
...
### 3.2 Frontend: deferred to a later phase, kept for reference
| Concern | Choice |
|---|---|
...
- `architecture/ARCHITECTURE.md` (the design, derived from this plan) and a README with the demo script.

---

## 4. Software architecture

### 4.1 Shape: two services, one client bridge
```
...
- **Ingestion boundary:** seeds follow the canonical model (§4.6). Future import adapters would produce the same format. Not built now.
...
### 4.2 Repository layout (uv workspace)
```
sanovio/
...
### 4.3.1 Attribute registry: who owns templates, and how new attributes are added
...
### 4.4 Core idea: compare attribute by attribute
1. **Facts per side, produced where the data lives:**
   ...
   - **Effective record** of a variant = variant facts over family facts (§4.5). Results are cached by content hash.
2. **Current product (node, optional, set from search results):** in the single candidate search (§4.13), the purchaser can mark a result as **"this is our current product"**.
   ...
   - Always chosen by the purchaser. Identifier matches from product hints (§4.13) only highlight a candidate, never link it.
...
6. **Missing data and who gets asked:**
   ...
   - **Unknown in the requirement** → question to the **purchaser**, answered at the node and returned in a new requirement (§4.7). Or the purchaser marks the current product.
   ...
   - Up to 3 extra concerns outside the template. A question raised for one of them starts an attribute proposal (§4.3.1).
7. **Guardrails:**
   ...
   - Every requirement is validated at the hub against the current template schema (unknown fields rejected). Hub assertions are signature-checked (§4.15).

### 4.5 Enrichment: add facts, never overwrite
...
### 4.6 Data placement and storage (full column types and example rows: `architecture/data-model.md`)
...
| Facts | `article_facts` | attribute **and identifier** values with sources; only ever added to (identifiers merged in round 7) |
...
| Templates | `template_versions` | the current definition per category, synced from the hub by the client (D52: unsigned, no history — the name is kept for continuity with `architecture/data-model.md` N.11) |

Trust material is not in the node database: the node's private key and the hub's public key are files loaded at startup (§4.15).
...
- **Hashes** (core: SHA-256 over canonical JSON):
  - `record_hash` covers all resolved attributes of a projection row **and its identifier facts** (round 7), so a newly answered GTIN changes the supplier `record_hash` and the next round is not rejected as "no progress" — it is exactly the round in which `identifier_evidence` may now return `SAME_TRADE_ITEM`.
  ...
  - **The no-progress check runs at the hub** on `requirement_hash` + supplier `record_hash` (§4.7, H.14).
...
- **Prototype:** deterministic search on the hub projection (§4.13); the node offers simple local article search (`q` on name/internal ID).
...
### 4.7 Assessment state machine (hub) and when the loop stops
```
...
```
(Rendered as a proper state diagram in `charts/08`.)
...
### 4.8 Background job flow (per service)
...
### 4.9 LLM pipelines (tiered models) and who pays
...
### 4.10 API (both `/api/v1`, REST, typed through OpenAPI)
...
- **Purchaser (exchanged token, tenant-scoped):**
  - `GET /suppliers`
  - `POST /search {requirement, supplier_id?, limit?}` (§4.13; D54)
...
### 4.11 Frontend screens (later phase, reference only)
...
### 4.12 The full loop as REST calls (scenario 1)
...
| 2 | P | N `POST /articles/art_03/requirement` → H `POST /search {requirement}` (§4.13) | egress row; one ranked list across suppliers: Injekt 4606728V and Omnifix near the top, Plastipak 300912 close behind. Requirement not stored at H |
...
### 4.13 Candidate search: one `POST /search` at the hub, run twice around "current product"
...
1. **At the node (`POST /articles/{id}/requirement`):**
   ...
   - Build the requirement through the allowlist (§4.14), write the egress row, and return it with a `category_unconfirmed` warning.
2. **At the hub:**
...
6. **Pre-check each candidate** with the core comparators: MATCH 1 / ACCEPTABLE_DEVIATION 0.7 / UNKNOWN 0.3 / MISMATCH 0, weighted critical 3 · major 2 · minor 1, normalized to 0–1.
   ...
   - Provisional attribute values go into `additional_information` (§4.3.1).
7. **Response 200:**
...
9. **Row actions** (two):
    ...
    - **Start assessment:** N `POST …/requirement` → H `POST /assessments {requirement, variant_id}` → loop §4.12.
...
### 4.14 The requirement: the only hospital data that leaves the node
...
| identifier keys | — | identifier schemes are a fixed list in the core (`GTIN`, `EAN`, `MANUFACTURER_REF`, `PHARMACODE` at the node; `GTIN`, `SUPPLIER_ARTICLE_NO`, `PZN`, `HIMIV` at the hub), **not** registry lookups, because the node has no copy of the registry (§10 finding 9) |
...
### 4.15 Identity, keys and secrets
...
### 4.16 Network and deployment modes
...
## 5. Seed data and demo scenarios
...
## 6. Decisions not fixed by the brief (why, and what would change them)
...
| D28 | **Hub assessments owned by the hospital tenant; creator, resolver and assignee recorded as pseudonymous principals; names resolved at the node** (review round 6) | Owned by users; assignee and verdict copy in a node table (`assessment_links`) | Staff change; the hub holds pseudonyms only, never staff identities; one source of truth avoids a node copy that goes stale; every purchaser of the hospital sees the same list. | Four-eyes approval → approver role, resolver principal ≠ creator principal |
...
| D34 | **Plain requirement (JSON) built by allowlist at the node, strict template schema at the hub, opaque `article_ref`, issuance log** (review round 5) | Node-signed envelope (JWS); send the article record | Minimization comes from the allowlist and the hub schema, not from a signature. The client already receives all article data, so a signature would only add protection against injection with a stolen hub token (which can already read and resolve that hospital's assessments) and provable egress completeness. Saves ~½ day; the stable `article_ref` still lets the hub dedupe and show past verdicts without knowing the article. | Hospitals require provable egress control → wrap the same payload in a node signature (~½ day) |
...
| D37 | **No hub-signed snapshots or verdicts: the node stores current-product values as reported by the client; verdicts stay at the hub only** (review rounds 5–6) | Hub-signed variant snapshots and verdict attestations | A purchaser can enter any value as a correction anyway, so a signature would only protect the "reference item" label; verdicts need no node copy (D28, D49). Saves ~½ day. | Hospital needs a tamper-evident decision record → hub-signed export (D49) |
...
## 7. Assumptions used until the client replies
...
When the client replies, update §6/§7 and `architecture/ARCHITECTURE.md`.

---

## 8. Implementation plan (≈5½ days in seven chunks, backend only)

> **Timebox reality check.** Your timebox is 3–5 days. Before the §10 review the plan was ≈6½ days; D52–D55 remove about 1¼ days of *build* work, and D42/D56 add back ¼ for the node's ingestion pipeline, landing at **≈5½**. The earlier "≈4¼" counted the savings twice — against the headline and again inside the phases — so this section now states what the chunks actually add up to.
> **To get under 5 days, take one more cut:** the attribute registry (chunk 5's proposals → provisional → approve path, plus scenario 7) is ~½ day and is the only remaining piece that no other scenario depends on → **≈4¾**. It is also the piece the client explicitly asked about (Q20/Q21), so cut it last and knowingly.

**How the chunks are meant to be used.** Each one is independently implementable and independently testable: it ends with a command you can run and a result you can look at. Chunks 0–2 produce a **working hospital node with no hub in existence**, which is the order you asked for. The hub only appears in chunk 4, and the first cross-service traffic happens there.

**Dependency note that makes node-first possible:** the node needs a category template, which normally comes from the hub. In chunk 2 the node seeds its template definition straight from the core YAML (`equivalence_core.templates.seed`). Chunk 4 adds the hub as the source and the client-side sync; the node's storage and API do not change when that happens.

---

### Chunk 0 — Workspace and tooling (½ day)
- uv workspace (`packages/equivalence-core`, `apps/hospital-node`, `apps/supplier-hub`, `tools/demo-client`), `uv python pin`, one committed `uv.lock`.
- Dependencies per member (§3.1); **both** apps get `anthropic` + `jinja2` — the node for its one ingestion pipeline (D42, D56).
- Settings + secrets loader per service, `/api/v1/health` on both, import-linter contracts, `make keys` (node key pairs only, D52), `make dev`, `make lint`.
- **Done when:** `make dev` serves `:8001/docs` and `:8000/docs`; `make lint` passes including the import contracts; `make keys` writes two node key pairs at mode 0600.

### Chunk 1 — Core: what the node needs (½ day)
Pure Python, no service, no database.
- `TemplateDefinition` model + YAML seed loader; the fixed identifier-scheme list per side.
- Parsers: German decimals, `×` dimensions, inch fractions, gauge ↔ outer diameter, packaging; synonym dictionaries.
- GS1 mod-10 validator and the cross-identifier data-quality checks.
- `facts.py`: precedence per side, identifier facts kept as a set, canonical JSON, `record_hash`, `definition_hash`.
- `exchange/`: strict `RequirementPayload` (`extra="forbid"`) + `requirement_hash`; `HubAssertion` claims; `jws.sign` / `jws.verify` (Ed25519 pinned, `typ`/`aud`/`exp`/`iat`, `kid` bound to issuer).
- **Done when:** `uv run pytest packages/equivalence-core` is green on the real strings from §9 — "0,80 mm x 40 mm", "21 G x 1 ½\"", "10 ml, nutzbar bis 12 ml" — the 10 CSV identifiers (3/10 GTIN valid), hash stability, and every JWS rejection case.

### Chunk 2 — Hospital node, complete and demoable on its own (1 day)
The first thing you can show someone. No hub exists yet.
- Models N.1–N.7 + N.11 (no `jobs`, no `llm_calls`, D53/D55) + Alembic migration; seed the 10 CSV articles with their identifiers as facts.
- Auth: local accounts, hashed bearer tokens, roles; `hub_subject_id` per user.
- Normalization at ingestion (D56): parsers + one batched `normalize_article` call (FakeLLM in tests), merged with the parsers winning on numbers; projection rebuilt in the same transaction. Startup re-normalizes only articles whose `content_hash ≠ normalized_hash`; a later single edit normalizes inline.
- Articles API: list/search, detail with sources and unknowns, category confirm, fact corrections, identifier values through the same facts endpoint.
- Requirement builder (allowlist from the projection) + `egress_log` + per-user rate limits and alerts; `POST /hub-assertions`; `GET /egress`; `GET /users`; `cli keygen`, `cli seed`.
- Template definition seeded from the core YAML (see the dependency note).
- **Done when:** `make demo node` (a node-only script) logs in, lists the 10 articles, shows art_03's resolved attributes and unknowns, prints the requirement JSON with **no** name, brand, identifier, price or quantity in it, and prints the egress log with one row per issued object. Node tests green: seed shows 7/10 invalid GTINs and 9/10 `GTIN_EAN_MISMATCH`; the per-user limit returns 429; `hospital_node` imports no HTTP client.

### Chunk 3 — Core: comparison primitives (¼ day)
Still pure Python; separated from chunk 1 because nothing before this needs it.
- Comparators (exact, tolerance, same-or-finer, same-or-more, gauge ↔ diameter), `verdict_rules`, `identifier_evidence` (`SAME_TRADE_ITEM` | `NO_INFORMATION`, never a mismatch).
- **Done when:** the verdict table in §4.4 is reproduced by tests row for row, and `identifier_evidence` returns `SAME_TRADE_ITEM` only for a check-digit-valid match.

### Chunk 4 — Hub foundation: tenants, catalog, search (1 day)
First cross-service traffic: the node's assertions finally have somewhere to go.
- Models H.1–H.22 (H.9 removed) + migration; operator CLI; tenant and signing-key registration/revocation.
- Token exchange with every check in §4.15 and `used_assertion_jtis`; supplier login.
- Catalog seed (families + variants → facts through the core parsers); `normalize_item` against **FakeLLM fixtures**; projection rebuild.
- Attribute registry seeded from YAML; `GET /templates`, `GET /templates/{code}`, `GET /attributes`; `PUT /templates` sync at the node replaces the chunk-2 YAML seed.
- Requirement intake (strict schema, tenant from the token) and `POST /search` with product-hint matching.
- **Done when:** a real assertion minted by the node is exchanged for a hub token; a real requirement built by the node returns Plastipak 300912 and Injekt 4606728V but not Emerald 307736 (`excluded_by.connector`); marking the current product at the node and re-searching returns a `search_spec` with strictly more hard filters; replayed, expired and revoked assertions are all rejected.

### Chunk 5 — Hub loop: assessment, questions, answers (1¼ days)
- LLM layer: `LLMClient`, Anthropic adapter (per-pipeline model and effort, `messages.parse` + one repair retry, refusal fallbacks, `stop_reason`, caching, `llm_calls`), `FakeLLM`; prompts `normalize_v1`, `judge_v1`, `extract_answer_v1`, `propose_attribute_v1`, `simulate_supplier_v1`.
- Job queue + worker (hub only); assessment creation; ASSESS = `identifier_evidence` → comparators → judge → verdict rules → stop conditions on `input_hash`.
- Questions with addressee; purchaser answers returning as a new requirement; supplier inbox, drafts, batch submit; enrichment and extraction; re-judge chain.
- Resolve / override / manual / extra round / request more info / cancel / retry with optimistic locking; assignee; events.
- Attribute registry runtime: PROPOSE_ATTRIBUTE, provisional on send, `additional_information`, approve-only curation (D55). *This is the ½ day to cut if the timebox bites.*
- **Done when:** a FakeLLM integration test drives scenarios 1–3 to their expected states, fact scopes and events; a `SAME_TRADE_ITEM` round records **zero** judge calls; a supplier token can see neither requirements nor tenant names.

### Chunk 6 — Demo client, end-to-end, evals, docs (1 day)
- `tools/demo-client`: one session holding both tokens, silent re-exchange on 401, a script per scenario printing states, requirements, both search passes, the comparison table, questions, final facts and the egress log.
- `tests/e2e`: hub + node `ten_ksp` in-process, `ten_spital2` as a second tenant at the hub, scenarios 1–4, 6, 7.
- OpenAPI polish; `evals/golden.jsonl` + `make eval` (rules extraction, verdict accuracy, question recall, cost/latency per tier); security pass (log scrubbing, `.secrets/` permissions, CORS, thresholds); README with the demo script and the onboarding/rotation runbook.
- **Done when:** `make demo SCENARIO=1..4` works with `LLM_MODE=fake` and with a real key, and the e2e suite is green.

---

**Deferred by D52–D55** (all additive, none blocking the brief): template versioning and signed bundles · node job queue · search paging and `relax` · curation merge/reject · extra concerns · `.http` collections · second node in e2e. `EGRESS_DENY_ATTRIBUTES` stays: it is ten lines and it is the hospital's veto over egress.

## 9. Verification
- **Unit, core (`uv run pytest packages/equivalence-core`):**
  - parsers on real strings: "0,80 mm x 40 mm", "21 G x 1 ½"", "10 ml, nutzbar bis 12 ml", "25 x 100 Stück", "100 / 1.200", "Kanüle Sterican 0,8 × 40 mm"
  - synonyms: "BD Luer-Lok™" and "Luer-Lock" → `LUER_LOCK`; unknown spellings fall through to the semantic path; gauge/OD disagreement flag
  - GS1 validation on the 10 CSV identifiers (3/10 GTIN valid, 1/10 EAN valid)
  - comparators; precedence per side; family → variant; `record_hash` changes only when values change and is independent of key order
  - verdict table
  - template loader: the YAML seed and the hub's definition JSON produce identical `TemplateDefinition`s; a definition with an unknown type or rule is rejected
  - JWS (assertions): accepts valid tokens; rejects `alg:none`, HS256 with the public key as secret, wrong `typ`, wrong `aud`, expired, future `iat`, unknown `kid`, a `kid` of another issuer, tampered payload
  - `RequirementPayload` rejects extra keys; `requirement_hash` is independent of key order
- **Unit/integration, node (`uv run --package hospital-node pytest`):**
  - normalization of the 10 CSV names against an expected-facts fixture, with FakeLLM: parsers and model agree on volume/connector/sterile for art_03 and on OD/length/gauge for art_06
  - **merge rule:** when the model returns a different number-with-unit than the parser, the parser's value wins and the model's is discarded (D8, D42)
  - **D56:** `make seed` makes exactly one `normalize_article` call for ten articles; a restart with unchanged articles makes **zero**; serving a search or building a requirement makes zero; one edited article normalizes inline
  - `llm_calls` at the node records the call with tokens and cost, and its `request` never contains the API key
  - re-normalization keeps a `PURCHASER` category, may replace `RULES`/`LLM_SUGGESTED`
  - `data_quality_issues` (9/10 `GTIN_EAN_MISMATCH`)
  - **requirement allowlist:** for every seeded article, the serialized requirement contains none of the article's name, brand, internal ID, identifier values, price or quantities (substring search), and only template keys marked `shareable: true`. With product hints on, only brand, manufacturer article no. and a checksum-valid GTIN are added.
  - every requirement and assertion writes exactly one egress row with a matching content hash; exceeding the per-user limit → 429 + alert
  - current product: the preview lists fills and conflicts; `PUT` without a choice for a conflicting attribute → 422; `PURCHASER_ANSWER` facts are never overridden; undo restores the previous projection; **no identifier fact is created by the link** even when the hub variant has a GTIN
  - identifier facts: a seeded GTIN lands in `article_projection.identifiers`, never in `attributes`; it is absent from every requirement while hints are off, and changing it changes `record_hash` but not `requirement_hash`
  - `GET /articles?article_ref=` returns only this hospital's articles; `GET /users` requires a node token and returns display names with `hub_subject_id`
  - no HTTP client importable from `hospital_node` (import-linter)
  - template install: a new definition rebuilds that category's projections and adds the new attribute to `unknown_attributes`; a denylisted attribute appears only in `withheld_attributes`
- **Integration, hub (`uv run --package supplier-hub pytest`, FakeLLM + worker):**
  - token exchange: valid → token; replayed `jti` → 401; expired → 401; revoked `kid` → 401 and existing tokens from it revoked; unknown tenant → 401
  - requirement with unknown fields → 422; unknown `template_code` → 422; the tenant is always taken from the token
  - scenarios 1–3 via HTTP with fixture requirements; identical re-submitted requirement with no supplier change → NEEDS_MANUAL_DECISION (`NO_PROGRESS`)
  - a second `POST /assessments` for an open (tenant, `article_ref`, variant) → 409 with the existing ID
  - two-pass search: the first search on the thin art_03 requirement and the second on the enriched one hit the same endpoint; the second has strictly more hard filters, ranks Plastipak higher, and `like_variant_id` is not an accepted field (422)
  - product hints: a valid GTIN match sets `identifier_match` and ranks the candidate first without linking; no hints → no identifier matching
  - `send-questions` 409 while PURCHASER questions open
  - assignee: `PUT /assessments/{id}/assignee` stores a principal of the caller's tenant (event `ASSIGNED`); `assigned_to=me` returns only the caller's assessments; another tenant's assessment → 404
  - tenant isolation: `ten_spital2` principal gets 404 on `ten_ksp` assessments and its search flags never show `ten_ksp` verdicts
  - supplier sees alias only, never requirement contents; incomplete batch submit rejected; "applies to family" visible on sibling variants
  - duplicate-job protection; concurrent resolve conflict
  - `llm_calls.request` never contains the API key; judge prompt contains no `article_ref` or tenant name
  - registry: free question matching an existing attribute gets that key (FakeLLM fixture); `send-questions` 409 while a proposal is pending; proposal labels containing a tenant name, alias, brand or product name are rejected
  - identifier facts: an identifier answer creates an identifier-typed `item_facts` row with the correct `checksum_valid` and lands in the projection's `identifiers` section, never in `attributes`; it appears in no `comparator_results`, no `unknown_attributes`, no question and no judge prompt; a free question asking for a GTIN resolves to the `gtin` identifier definition and creates no comparable attribute
  - `identifier_evidence` (D51): matching check-digit-valid GTINs on both sides (hints on) → `SAME_TRADE_ITEM`, verdict EQUIVALENT, **zero** judge calls in `llm_calls`; differing GTINs → `NO_INFORMATION` and a verdict byte-identical to the same run with hints off; a matching GTIN with an invalid check digit is ignored and raises a data-quality warning
  - provisional facts: visible to another tenant as `additional_information`; the rule verdict is identical with and without them; approval adds the attribute to the definition with the chosen criticality; rounds already judged keep the definition stored in their `input_snapshot`
  - suppliers and purchasers get 403 on curation endpoints
- **End-to-end (`uv run pytest tests/e2e`):** hub + one node + demo client (second tenant at the hub), scenarios 1–4, 6, 7; after scenario 1 the node egress log lists exactly the expected objects; after scenario 7 the second node shows `peel_off_label` as unknown and BD is not asked again.
- **LLM quality (`make eval`, real API):** ≥90% attribute extraction on the 10 German names, reported for **`rules` alone and for `llm` + parsers** so the added value of the node call is measurable (client question 16); ≥85% verdict accuracy, 100% of critical gaps asked; results stored per prompt version, model tier and normalization mode.
- **Manual demo (REST):**
  1. `make setup && make keys && make seed && make dev`.
  2. `make demo SCENARIO=1`, then repeat by hand in the two Swagger UIs: node login → hub assertion → hub token exchange (Authorize) → requirement → search → mark current product → automatic second search → assessment → questions → supplier token in the hub Swagger → answers → resolve → assessment list joined with local article and user names.
  3. Scenarios 2–4.
  4. Check: `GET /egress` on the node, issued requirements, identifier warnings (node only), `excluded_by`, fact sources, event timeline, `llm_calls`.

---

## 10. Pre-implementation review (critical pass)

The plan had grown by accretion over eight review rounds. This pass asked of every part: is it load-bearing for the brief, can it go, is it overengineered, and does it contradict anything else. Verdicts are applied above; the reasoning is kept here so nothing is silently dropped.

**The headline problem was scope.** The timebox is ~3–5 days; the plan had reached ≈6½. Four scope decisions (D52–D55) remove about 1¼ days of build work and land at **≈5¼ days** (§8), without touching anything the brief asks for.

> **Correction.** This section first claimed ≈4¼. That subtracted the savings twice — once from the headline and again from phases that had already been shortened — while the per-phase numbers still summed to 6½. §8 now states the chunk sum directly and names the one remaining cut (the attribute registry, ~½ day) that gets under 5 days.

### What is load-bearing (keep, no changes)
| Part | Why it cannot go |
|---|---|
| Facts (append-only, sourced) + projections | The whole liability story: who said what, when, replayable judgments. Also what makes enrichment meaningful (D12, D26) |
| Comparators + verdict rules in code | The difference between this and "ask an LLM to compare two paragraphs". Units, gauges and decimal commas are exactly where models slip (D7, D8) |
| Requirement allowlist + egress log | The reason the split exists. Without it, "hospital-hosted data" is a claim, not a property (D34, D48) |
| Token exchange (node-signed assertions) | The only mechanism that lets a purchaser reach the hub with no shared secret and no node→hub connectivity (D36) |
| Question loop with addressee + stop conditions | This *is* the brief: ask, enrich, repeat, terminate (D11, D16) |
| Provisional attributes shared across hospitals | An explicit client requirement (Q20/Q21, D43–D44) |
| Identifier facts + check digits | The sample data's defining property: 7/10 GTINs invalid (D10, D31, D50) |

### What was overengineered (now deferred)
| # | Part | Verdict | Saving |
|---|---|---|---|
| 1 | Template **versioning**: signed bundles, hub signing key + JWKS, node version history, pinning, current+previous acceptance, `upgrade_template`, downgrade refusal | **Simplify** → unsigned sync of one current definition per category (D52). The registry requirement (new attributes without a release) is fully preserved; only the version machinery goes | ~1 day |
| 2 | Node **job queue + worker thread** | **Remove** (D53). Ten articles, rules-based normalization in milliseconds; a queue here buys nothing and costs the most confusing error path in the demo (`NOT_NORMALIZED` + `retry_after_s`) | ~½ day |
| 3 | Search `relax`, `cursor` / `next_cursor`, `relaxation_hints` | **Remove** (D54). Paging and relaxation over a 25-variant seed catalog is scaffolding for a problem the prototype does not have | ~¼ day |
| 4 | Curation merge/reject, extra concerns, `.http` files, second node in e2e | **Defer** (D55) — these were already the cut list; making them default scope rather than a fallback is what brings the estimate inside the timebox. *Node `llm` normalization was on this list and has since been restored (D42, D56): the user judged the extraction quality worth the ¼ day and the hospital-held key.* | ~½ day |

Those savings are scope removed from the *whole* effort — build, tests, demo scripts and docs. Against the per-chunk build numbers in §8 they net out at about 1¼ days, which is why the honest total is ≈5¼ rather than 6½ − 2½.

### UML that gets simpler as a result
- **05a / 07a (node):** the `Templates` and `Infrastructure` packages disappear — no `jobs`, no `llm_calls`, and `template_versions` becomes a two-column cache. The node model drops from 9 tables to 7.
- **06 (application classes):** `TemplateBundle` goes; `Jws` is left with one caller (assertions); `RateLimiter` merges into `EgressLog`, which already owns the table it counts.
- **18 (key lifecycle):** loses its entire hub-key half — one key pair in the system instead of two.
- **10 (job states):** becomes hub-only.
- **12 and 17** overlap heavily (17 is the node half of 12). Merging them would take the set from 21 to 20 diagrams; deferred, since both are already rendered and correct.

### Inconsistencies found and fixed
1. §Context still described the client sending a "find-similar request" (removed in round 8).
2. Round-3 notes listed "local links to assessments" as node content (removed in round 6).
3. Status line claimed rounds 5–7 (round 8 was applied).
4. §4.3.1 reused `NOT_NORMALIZED` for the post-template-install rebuild, but that guard compares `normalized_hash` with `content_hash` — which does not change when a template is installed, so it would never fire. Now a distinct `TEMPLATE_REBUILD_PENDING`.
5. §8 heading said ≈6¾ days while its own timebox block said ≈6½.
6. Phase 3 said "Models H.1–H.22" after H.9 had been removed.
7. Phase 5 still listed a "find-similar" action in the demo client.
8. The appendix pointer said the round 7 chart update was pending after it had been completed and verified.
9. **N.5 said a node fact's `attribute_key` must be "an identifier definition (`kind = IDENTIFIER`)" — but the node has no copy of the registry.** Identifier schemes are now a fixed list in the core, stated in §4.14.
10. `GET /articles` was documented twice with different framings.
11. **§8's phase durations never summed to its own headline** — the phases added to 6½ while the heading claimed ≈4¼, because D52–D55 were applied to the prose and not to the numbers. Found while restructuring §8 into chunks; the section now reports the real sum.

### Round 9: applying D52–D55 to the schema and the diagrams

**Done so far**

| Target | Applied |
|---|---|
| `architecture/data-model.md` | N.9 `jobs` and N.10 `llm_calls` replaced by a removal note (D53, D55); N.11 reduced to the definition cache (`code` unique, `definition`, `definition_hash`, `hub_updated_at`); H.21 reduced to one current definition per category; H.14 `input_hash` now uses `definition_hash` |
| `charts/src` (12 files) | 01, 02, 03, 05a, 06, 07a, 10, 11, 12, 13, 18, 19 — node worker, `Job` / `LlmCall`, `NodeJobKind`, `TemplateBundle`, `RateLimiter` (folded into `EgressLog`), the node `jobs` / `llm` packages and the hub signing key are gone; template sync is unsigned; search lost `relax` and `cursor` |
| `architecture/charts.md` | Round 9 table recorded |

**Still to do** (found by a second sweep; blocked while in plan mode)

| Target | Remaining |
|---|---|
| `architecture/data-model.md` | N.6 `template_version` → `definition_hash`; H.12 and H.13 keep `template_code` only (no pinning); H.14's `template_version` column → `definition_hash`; drop `TEMPLATE_UPGRADED` from the H.17 event types |
| `charts/src/02` | Core `exchange` label still lists the template bundle; `RequirementIntake` still says "versions"; `AttributeRegistry` still says "versions, signed bundles" |
| `charts/src/06` | `TemplateSyncService.versions()/install(bundle)`, `CandidateSearchService.search(… relax, limit, cursor)`, `TemplateService.draft/publish/bundle/accepted_versions`, `TemplateService ..> Jws : sign bundle`, `RequirementIntake ..> TemplateService : accepted versions` |
| `charts/src/08` | The pinning note and `input_hash` still mention a template version |
| `charts/src/12`, `17` | The `TEMPLATE_VERSION_UNSUPPORTED` branch and the "unsupported template version" alt |
| `charts/src/05b`, `07b` | `template_version` on `Assessment`, `Requirement` and `assessment_rounds`, plus the closing note |
| render | `bash charts/render.sh`, then check each changed PNG |

**Verified already:** the three regex-deleted blocks (05a `Infrastructure`, 07a `jobs`/`llm_calls`, 19 publish) closed cleanly, and the brace-count warning on 07a/07b is a false positive of the check — 07b was untouched in round 9 and shows the same ratio.

### Accepted as-is, with the trade-off stated
- **`identifier_evidence` at the hub only fires when a hospital enables product hints**, which is off by default — so in the default demo the hub stage returns `NO_INFORMATION` every time and the client-side check does the visible work. Scenario 4 deliberately re-runs with hints on to exercise it. Keeping the hub stage is still right: it is ~2 hours, and without it the rule lives only in a client we are not building yet.
- **Two services for a prototype** remains the user's decision (D32) and is not revisited here.

---

## Appendices (moved out of this document)

| File | Contents |
|---|---|
| [`architecture/data-model.md`](/home/ratlinghitman/code_stuff/sanovio/architecture/data-model.md) | **Appendix A** — conventions and typed value shapes (A.0), hospital node tables N.1–N.11, supplier hub tables H.1–H.22, each with column types and example rows |
| [`architecture/charts.md`](/home/ratlinghitman/code_stuff/sanovio/architecture/charts.md) | **Appendix B** — the 21 PlantUML diagrams in `charts/`, the per-round update tables (rounds 7 and 8 done) and their done-criteria |

Section IDs are unchanged, so references to N.5, H.13, A.0 or "Appendix A / B" anywhere above resolve into those two files.
